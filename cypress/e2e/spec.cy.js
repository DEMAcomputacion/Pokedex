describe("Comprueba el renderizado de los elementos de la pagina", () => {
...
describe("Comprueba funcionalidad y fallos del buscador", () => {
  ...
  it("se asegura que el buscador falle con un caracter no válido", () => {
    cy.get("#nombreONumeroPokemon").clear().type("#");
    cy.get("#buttonBuscar").click();
    cy.get("#mensajeError").contains("Solo se aceptan letras o numeros");
  });

  it("se asegura que el buscador falle con una cadena con caracteres no válidos", () => {
    cy.get("#nombreONumeroPokemon").clear().type("pok#emon");
    cy.get("#buttonBuscar").click();
    cy.get("#mensajeError").contains("Solo se aceptan letras o numeros");
  });
  ...
  it("se asegura que se muestra la advertencia al buscar un nombre de pokemon no valido", () => {
    cy.get("#nombreONumeroPokemon").clear().type("BELCEBU");
    cy.get("#buttonBuscar").click();
    cy.get("#mensajeError").contains("No pudimos encontrar ese Pokemon");
  });
});

describe("Comprueba el funcionamiento del paginador", () => {
  it("se asegura que se actualize el listado al dar click en siguiente pagina", () => {
    cy.get("#item1 > .card > .card-body > .nombre-pokemon-listado").then(
      ($btn) => {
