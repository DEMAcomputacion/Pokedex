...
import { cargarPokemon } from "../services/pokemon.js";
const OFFSET = 20;

const $formBuscar = document.querySelector("#buttonBuscar");
$formBuscar.onclick = buscarPokemon;

export function mostrarListadoPokemones(listado, nroPagina = 0) {
  ...
  const callbackOnClickContenedorTarjeta = (index) => {
    return mostrarUnPokemon(index + 1 + multiplicadorDePagina);
  };
  ...
  fichas.forEach((elem, index) => {
    ...
    $divContenedorTarjeta.onclick = () => {
      callbackOnClickContenedorTarjeta(index);
    };
    const $fotoPokemon = document.createElement("img");
    $fotoPokemon.src = "./img/cargando.gif";
    api.cargarFotoPokemon(index, multiplicadorDePagina);
    ...
    $nroPokemon.innerText =
      "Nro: " + (index + 1 + multiplicadorDePagina).toString().padStart(4, 0);
...
export async function mostrarUnPokemon(nroPokemon = 1) {
  ...
  try {
    ...
    $fotoGrandePokemon.src =
      (await unPokemon.sprites.other.dream_world.front_default) ||
      "./img/pokebola-no-foto.png";
